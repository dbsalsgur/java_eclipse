...
public class EmployeeDAO {
	...
	public EmployeeVO getEmployeeCheck(int no, String name) throws Exception {
		...
		try {
			...
			if (rs.next()) {
				retval = new EmployeeVO(rs.getInt(1), rs.getString(2), rs.getString(2), rs.getInt(4), rs.getString(5));
			}
	...
	public ArrayList<EmployeeVO> getEmployeetotal() {
		...
		try {
			...
			rs = pstmt.executeQuery();
			while (rs.next()) {
				emVo = new EmployeeVO(rs.getInt(1), rs.getString(2), rs.getString(2), rs.getInt(4), rs.getString(5));
				list.add(emVo);
			}
		} catch(SQLException se) {
			System.out.println(se);
